...
def getnet(datain,dataout,modulation):
    with tf.Graph().as_default():
        ...
        # Parameters
        learning_rate = 0.001
        training_epochs = 10000 #50000
        batch_size = 100
        display_step = 10
        # Network Parameters
        n_hidden_1 = 4 #int(input_num / 4.0) # 1st layer number of features
        n_hidden_2 = 4 #int(input_num / 4.0) # 1st layer number of features
...
for i in range(9):
    ...
    for inp in test_i:
        ...
        c = 0
        for n in nt:
            v = n[0].run(n[1],feed_dict={n[2]: [inp]})[0][0]
            p[c] = v
            c += 1
