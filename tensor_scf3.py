...
def scf2(y):
    ...
    for zz in range(o.shape[0]):
        dat.append(o[zz][np.argmax(o[zz])])
    return dat
...
if load_scf_training:
    ...
    train1,train_out = load_data("data-20samp/a",True)
...
if load_scf_testing:
    ...
    print("loading rnd 3 ")
    test,test_out = load_data("data-20samp/b",False)
...
def getnet(datain,dataout,modulation):
    with tf.Graph().as_default():
        ...
        # Parameters
        learning_rate = 0.001
        training_epochs = 100000 #50000
        display_step = 10
        n_hidden_1 = 60 #int(input_num / 4.0) # 1st layer number of features
        n_hidden_2 = 60 #int(input_num / 4.0) # 1st layer number of features
        n_input = len(datain[0][0])
        n_classes = 4 #len(mod) 
        ...
        def multilayer_perceptron(x, weights, biases):
            ...
            out_layer = tf.matmul(layer_1, weights['out'],name="out") + biases['out']
            return out_layer
...
nett,pred,x = getnet(train_,train_out,None)
...
for i in range(9):
    ...
    allv = 0
    for inp in test_i:
        p = nett.run(pred,feed_dict={x:[inp]})
...
for i in range(2):
    ...
    for i2 in range(len(train_[i])):
        ...
        print(train_out[i][i2])
        
        if (train_out[i][i2] == md).all():
            to.append([1.])
            print("FSK")
            b1 += 1
        else:
...
with tf.Graph().as_default():
    ...
    net1 = tflearn.input_data(shape=[None,len(train_[0][0])])
    net1 = tflearn.fully_connected(net1, hidden,activation='sigmoid') #, activation='sigmoid')
    net1 = tflearn.fully_connected(net1, 1, activation='sigmoid')
    regressor = tflearn.regression(net1, optimizer='adam',learning_rate=0.001,loss='mean_square') #, loss=lossv)
    m = tflearn.DNN(regressor,tensorboard_verbose=3)
    m.fit(t, to, n_epoch=1000,validation_set=0.0,shuffle=True, snapshot_epoch=False,show_metric=True)
    ...
    for snr in range(9):
        ...
        gd = 0
        for val in test[snr]:
            #print m.predict([val]),test_out[snr][c]
            pred = m.predict([val])[0][0]

            if pred < 0.5 and not (test_out[snr][c] == md).all():
                gd += 1
            if pred >= 0.5 and (test_out[snr][c] == md).all():
                gd += 1


        
            #if  m.predict([val])[0] < 0.5 and not (test_out[snr][c] == [0.,0.,0.,1.]).all():
            #    gd += 1

            c += 1
        print "SNR",snr,float(gd)/float(c),"total ",c
